...
float wass2(float* f, float* g, float* x, float*p, int N, int P){
    ...
    sum = 0.0;
    for(ix = 0; ix < N - 1; ix++){
        sum = sum + 0.5 * (x[ix+1] - x[ix]) * (y[ix+1] + y[ix]);
    }
...
int main(int argc, char* argv[]){
    ...
    sf_file qfpos_file, qfneg_file, qgpos_file, qgneg_file;
    float ***F_pos, ***F_neg, ***G_pos, ***G_neg, *f_pos, *f_neg, *g_pos, *g_neg, ***QFpos, ***QFneg, ***QGpos, ***QGneg;
