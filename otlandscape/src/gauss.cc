...
//    assert( n_cases == ng_cases ); 
    ...
    float** vals;
    vals = sf_floatalloc2(n_cases, nx);
    sf_file output_file;
    ...
    cerr << "loop\n";
    //compute distances
    for(int iz = 0; iz < n_cases; iz++){
        for(int ix = 0; ix < nx; ix++){
            float value = 0.0;
            gslice curr_slice(start_index,
                lengths_arr,
                strides_arr);
            start_index += nt;
            ...
            if( mode == 0 ){
               ...
               if( mode == 1 ){
                   ...
                   WassSplit2<float> my_misfit(g_vec, t_vec, p_vec, 1);
